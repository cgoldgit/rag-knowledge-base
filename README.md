...
- **多用户多会话**：每个用户拥有独立会话，互不干扰；支持**置顶**、重命名、清空、删除（三点菜单操作）
...
- **重排序（Rerank）**：BGE-reranker 模型对候选片段精排，提高回答准确率（失败自动降级）
- **流式输出**：回答逐字返回（打字机效果），降低等待感（中断自动保存不丢数据）
- **多轮对话理解**：自动结合上下文理解追问（如"那充电呢？"）
- **检索兜底**：知识库无相关信息时明确告知，绝不编造
- **系统身份识别**：询问模型身份时正确回答（基于 DeepSeek 构建）
- **Redis 问答缓存**：热门问题 5 分钟内秒回，不重复调用大模型
- **接口限流**：Redis 原子计数防刷（每用户每分钟 10 次，故障时放行）
- **回答评价**：点赞/点踩，可统计回答质量
- **自动重试**：向量化调用失败自动重试（指数退避）
- **故障降级**：向量服务异常时自动降级为关键词检索，不中断服务
- **单元测试**：81 个测试点全部通过（安全/解析/检索/问答/缓存）
- **权限控制**：知识库管理仅管理员可用
...
**双击项目根目录 `start.bat`** —— 自动启动后端 + 前端 + 打开浏览器，无需手动命令。
...
| 版本 | 说明 |
|------|------|
| v1.3 | 系统身份回答、一键启动脚本（start.bat）、单元测试扩充至 81 个、文档更新 |
| v1.2 | 会话置顶 + 三点菜单交互 + 安全修复（JWT 时区/路径穿越）+ 单元测试 |
